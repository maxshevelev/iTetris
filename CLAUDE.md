...
- **Event processing is order-independent.** `GameViewModel.apply()` uses a two-pass collector-then-apply pattern. Pass 1 gathers values without side effects; Pass 2 applies in a strict logical order (dimensions → grid snapshot → grid → piece → …). Never rely on `Set` iteration order. **Within a tick, event types must be applied in a strict order — e.g. `.linesCleared` before `.grid` so the animation snapshot captures the pre-clear grid while it's still the current state.**
...
- **iOS gestures.** Dynamic-zone system via `GestureHandler`: zones computed relative to active piece horizontal position (center zone min 3 cells, width = `max(cellSize * 3, cellSize * span)`). On touch begin (first `onChanged` of `DragGesture(minimumDistance: 0)`), intent (left/right/rotate) is locked based on finger X vs zone boundaries; intent persists for entire touch regardless of finger/piece movement. Zone layout is captured at gesture start (`gestureStartZoneLayout`) as fallback — if `onChanged` doesn't fire for a very quick tap, `onEnded` resolves intent from the captured layout using `startLocation`. All intents fire on release regardless of finger movement (no `minimumSwipeDistance` check) if no hold was active. DAS = 170ms, ARR = 50ms (NES Tetris standards). Hold auto-repeat uses locked intent — **rotate does NOT auto-repeat** (`holdStart()` returns early for `.rotate`). `LongPressGesture(minimumDuration: DAS, maximumDistance: 500)` for ARR loop (high max distance so drags don't cancel it). Swipe down = hard drop (priority, fires once per gesture). `resetForNewPiece()` on `pieceBlocks` change to reset `hasHardDropped`, guarded by `isGestureActive`. Haptic feedback: light for move, medium for rotate, heavy for hard drop. Zone indicators: center zone is a `Rectangle` with alpha 0.04, side zones have no fill (only flash fill alpha 0.12 during gesture), icons alpha 0.12, vertically centered within the game grid with 20pt top/bottom insets. Toggleable via Settings → Display → "Zone indicators" (non-persistent).
...
| Review item 4 (Hard-drop overlay off-screen) | ✅ Done — not a real bug, aspect ratio + frame max bounds produce matching sizes |
